...
class UserService{
    ...
    static async Signup(userRequest) {
        ...
        await User.findOne({ username: userRequest.username.toLowerCase() }).then(user => {
            if (user) {
                reply = { status:'error', code:403, message:'Username already exists' };
            }
            else
            {
                User.findOne({ email: userRequest.email }).then(user => {
                    if (user) {
                        reply = { status:'error', code:403, message:'Email already in use' };
                    }
                    else
                    {
